...
case class App5Env(
  ...
  byDeploymentBuilder: StatefulFlowBuilder[EntranceCounterReading, App5SpekkaStatefulSharding.CounterIncremented, App5SpekkaStatefulSharding.GetCounter],
  byEntranceBuilder: StatefulFlowBuilder[EntranceCounterReading, App5SpekkaStatefulSharding.CounterIncremented, App5SpekkaStatefulSharding.GetCounter]
)
...
object App5SpekkaStatefulSharding extends AppSkeleton[App5Env, NotUsed] {
  ...
  implicit val counterIncrementedCodec = io.circe.generic.semiauto.deriveCodec[CounterIncremented]
  implicit val counterIncrementedSpekkaCodec = Utils.deriveSpekkaCodec[CounterIncremented]
  ...
  override def init()(implicit system: ActorSystem): App5Env = {
    ...
    val clusterSharding = ClusterSharding(akka.actor.typed.ActorSystem.wrap(system))

    val shardedRegistry = ShardedStatefulFlowRegistry(registry, clusterSharding, 30.seconds)
    
    def logic(prefixF: EntranceCounterReading => String) = StatefulFlowLogic.EventBased[State, CounterIncremented, EntranceCounterReading, GetCounter](
      () => State(0),
      (state, in) => {
        println(
            s"${Utils.prettyPrintTimestamp(in.timestamp)} - ${prefixF(in)} total entrances: ${state.counter + in.counter}"
          )
        ProcessingResult.withEvent(CounterIncremented(in.counter))
      },
    ...
    val backend = AkkaPersistenceStatefulFlowBackend.EventBased[State, CounterIncremented](
      AkkaPersistenceStatefulFlowBackend.EventBased.PersistencePlugin.CassandraStoragePlugin
    ).withEventCodec
    .withSnapshotCodec

    val byDeploymentProps = logic(e => s"deployment ${e.deploymentId}").propsForBackend(backend)
    val byEntranceProps = logic(e => s"deployment ${e.deploymentId} entrance ${e.entranceId}").propsForBackend(backend)

    val byDeploymentBuilder = shardedRegistry.registerStatefulFlowSync("byDeployment", byDeploymentProps)
    val byEntranceBuilder = shardedRegistry.registerStatefulFlowSync("byEntrance", byEntranceProps)
